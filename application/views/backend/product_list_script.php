<script>
	function comfirmDelete(dataID,table,product_name){  
       		...
       		swal({
                ...
                buttonsStyling: false
		   		...
		   		$.post('<?php echo base_url('Control/deleteData')?>', { dataID : dataID , table : table }, function(data){  
	...
	function setShow_onWeb(dataID,val2,table){
		...
		$.post('<?php echo base_url('control/set_ShowOnWeb2')?>' , { dataID : dataID , check : check , table : table }, function(data){
			if(data==1){
				//$('#ch'+dataID).val(check);
				swal({

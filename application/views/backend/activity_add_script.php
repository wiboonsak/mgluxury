<script>
	...
	function removeMe(imageID,imageName){
		 swal({
                ...
                buttonsStyling: false
		   		...
		   		$.post('<?php echo base_url('Control/deleteactivityImg')?>', { imageID : imageID , imageName:imageName }, function(data){ 
					console.log(data);
					   if(data=='1'){
						     swal({
								title: 'Deleted !',
	...
	//-----------------------------------
	function Addactivity(){  // news_title  news_detail
		...
		if(news_title==''){
			swal({
					title: 'warning !',
					text: "Please Enter Activity Topic.",
					type: 'warning',
					confirmButtonClass: 'btn btn-confirm mt-2'
			}) 
		...
		$.ajax({
		 type:'POST',
		 url:'<?php echo base_url('Control/Addactivity')?>',
		 processData: false,
		 contentType: false,
		 data : postData,
		 xhr: function(){
		  ...
		  // console.log("File Uploaded");
			...
			if(status=='success'){
				...
				swal({
                     ...
                     setTimeout(function(){ window.location.href = "<?php echo base_url('Control/activity_add/')?>"+data; }, 2000);
			}else{
					 swal({
	...
	function  loadImages(ProID){
		$.post('<?php echo base_url('Control/loadactivityImages')?>' , { ProID : ProID }, function(data){
			$('#showImage').empty();
			$('#showImage').html(data);
		   		...
		   		$.post('<?php echo base_url('Control/deleteyoutube')?>', { dataID : dataID , table : table }, function(data){  
					console.log(data);
					   if(data=='1'){
						  ...
						  setTimeout(function () {
                        window.location.href = "<?php echo base_url('Control/activity_add/') ?>"+currentID ;
                    }, 2000);
